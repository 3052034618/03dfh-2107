...
const MinePage: React.FC = () => {
  const { activities, userActivities } = useStore();
  const [user] = useState(mockCurrentUser);

  React.useEffect(() => {
    console.log('[Mine] User activities updated:', 
      'created:', userActivities.created.length, 
      'joined:', userActivities.joined.length);
  }, [userActivities]);

  const createdActivities = useMemo(() => {
    return activities.filter(a => userActivities.created.includes(a.id));
  }, [activities, userActivities.created]);

  const joinedActivities = useMemo(() => {
    return activities.filter(a => userActivities.joined.includes(a.id));
  }, [activities, userActivities.joined]);
          ...
          <View className={styles.section}>
            ...
            <View className={styles.miniActivityList}>
              {joinedActivities.length === 0 ? (
                <View className={styles.emptyActivity}>
                  <Text className={styles.emptyActivityIcon}>🎭</Text>
                  <Text className={styles.emptyActivityText}>还没有参与活动</Text>
                </View>
              ) : (
